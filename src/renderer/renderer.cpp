...
namespace tml {
    ...
    TmlRenderer::TmlRenderer(TmlWindow& window, TmlDevice& device) : tmlWindow{window}, tmlDevice{device} {
        recreateSwapChain();
    ...
    void TmlRenderer::freeCommandBuffers() {
        vkFreeCommandBuffers(tmlDevice.device(), tmlDevice.getCommandPool(), static_cast<uint32_t>(commandBuffers.size()), commandBuffers.data());
